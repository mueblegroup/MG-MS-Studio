<x-app-layout>
    <div class="p-6 sm:p-8 bg-gray-50/60 dark:bg-gray-900 min-h-screen">
        ...
        <div class="flex items-center justify-between mb-6">
            <div>
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400">
                    {{ $plan->name }} • {{ \Carbon\Carbon::parse($session->start_time)->format('Y-m-d H:i') }}
                    @if($session->session_name) • {{ $session->session_name }} @endif
                </p>
            </div>

            <a href="{{ route('teacher.plans.show', $plan->id) }}"
        ...
        @include('attendance.qr-link', ['kind' => 'plan'])
                            ...
                            <tr>
                                ...
                                <td class="px-4 py-4 text-right">
                                    <div class="flex justify-end gap-2">
                                        <form method="POST" action="{{ route('teacher.plans.sessions.attendance.mark', [$plan->id, $session->id, $u->id]) }}">
                                            @csrf
