        ...
        <div class="flex items-center justify-between mb-6">
            <div>
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400">
                    {{ $session->classModel->name ?? 'Class' }}
                    • {{ \Carbon\Carbon::parse($session->start_time)->format('Y-m-d H:i') }}
                </p>
            </div>

            <a href="{{ route('teacher.classes.show', $session->class_id) }}"
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                        @forelse($assignments as $a)
                            @php
                                ...
                                $ucc = $classCardByStudent[$a->user_id] ?? null;
                            @endphp
                            ...
                            <tr>
                                ...
                                <td class="px-4 py-4 text-right">
                                    <div class="flex justify-end flex-wrap gap-2">
                                        ...
                                        {{-- Class Card Attendance (-1) --}}
                                        @if($ucc)
                                            <form method="POST"
                                                  action="{{ route('teacher.classcards.usage.mark', [$session->id, $a->id, $ucc->id]) }}"
                                                  onsubmit="return confirm('Deduct 1 class from this student\\'s class card?')">
                                                @csrf
                                                <button type="submit"
                                        ...
                                        {{-- Normal Attendance --}}
                                        <form method="POST" action="{{ route('teacher.classes.attendance.mark', [$session->id, $a->id]) }}">
                                            @csrf
                                        ...
                                        <form method="POST" action="{{ route('teacher.classes.attendance.mark', [$session->id, $a->id]) }}">
                                            ...
                                            <button class="px-3 py-2 rounded-xl text-xs font-semibold
                                                text-white bg-red-600 hover:bg-red-700 transition">
                                                No Show
                                            </button>
                                        </form>
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="px-4 py-10 text-center text-sm text-gray-500 dark:text-gray-400">
                                    No students assigned to this class session.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
