<x-app-layout>
    <div class="min-h-screen space-y-6 bg-[#f7f2ea] dark:bg-gray-950">
        <div class="rounded-3xl border border-[#eadfce] bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900">
            ...
            <p class="mt-1 max-w-3xl text-sm font-medium text-[#6b5f52] dark:text-gray-400">Assign the platform plan, control access and manage renewal or expiry dates. A manual suspension remains locked until a superadmin restores another status.</p>
        </div>
        ...
        <div class="grid grid-cols-1 gap-6 xl:grid-cols-12">
            ...
            <div class="xl:col-span-8 rounded-3xl border border-[#eadfce] bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                ...
                <form method="POST" action="{{ route('superadmin.studios.update', $studio) }}" class="mt-6 grid grid-cols-1 gap-5 md:grid-cols-2">
                    ...
                    <label class="space-y-2"><span class="text-xs font-extrabold uppercase tracking-wider text-[#9a8c7d] dark:text-gray-500">Trial Ends At</span><input type="date" name="trial_ends_at" value="{{ old('trial_ends_at', optional($studio->trial_ends_at)->format('Y-m-d')) }}" class="w-full rounded-2xl border-[#eadfce] bg-white text-sm font-bold text-[#171717] shadow-sm focus:border-[#d97706] focus:ring-[#d97706] dark:border-gray-800 dark:bg-gray-950 dark:text-white"></label>
                    <label class="space-y-2"><span class="text-xs font-extrabold uppercase tracking-wider text-[#9a8c7d] dark:text-gray-500">Subscription Ends At</span><input type="date" name="subscription_ends_at" value="{{ old('subscription_ends_at', optional($studio->subscription_ends_at)->format('Y-m-d')) }}" class="w-full rounded-2xl border-[#eadfce] bg-white text-sm font-bold text-[#171717] shadow-sm focus:border-[#d97706] focus:ring-[#d97706] dark:border-gray-800 dark:bg-gray-950 dark:text-white"></label>
