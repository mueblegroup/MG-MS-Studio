            ...
            @if($tab === 'plans')
                @forelse($plans as $p)
                    <div class="min-w-0 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800 sm:p-5">
                        ...
                        <div class="mt-4 rounded-xl border border-gray-200 bg-gray-50 p-3 dark:border-gray-700 dark:bg-gray-900/50">
                            <div class="flex flex-wrap items-center justify-between gap-2">
                                <div>
                                    ...
                                    <div class="mt-1 text-[11px] text-gray-500 dark:text-gray-400">
                                        {{ $p->sessions->count() }} {{ \Illuminate\Support\Str::plural('session', $p->sessions->count()) }}
                                        @if($p->is_recurring)
                                            · Repeats {{ $p->recurrence_frequency === 'custom' ? 'every '.$p->custom_frequency_days.' days' : $p->recurrence_frequency }}
                                        @endif
                                    </div>
                                </div>
                                @if($p->teacher)
                                    <div class="inline-flex items-center gap-1.5 rounded-lg bg-white px-2.5 py-1.5 text-[11px] font-semibold text-gray-600 shadow-sm ring-1 ring-gray-200 dark:bg-gray-800 dark:text-gray-300 dark:ring-gray-700">
                                ...
                                @foreach($p->sessions as $session)
                                    <div class="rounded-xl border border-gray-200 bg-white p-3 dark:border-gray-700 dark:bg-gray-800">
                                        <div class="flex min-w-0 items-start justify-between gap-3">
                                            <div class="min-w-0">
                                                <div class="break-words text-xs font-bold text-gray-900 dark:text-white">
                                                    {{ $session->session_name ?: 'Plan session' }}
                                                </div>
                                                <div class="mt-1 flex flex-wrap gap-x-3 gap-y-1 text-[11px] text-gray-600 dark:text-gray-300">
                                                    ...
                                                    <span class="inline-flex items-center gap-1">
                                                        ...
                                                        {{ $session->start_time->format('h:i A') }}–{{ $session->end_time?->format('h:i A') ?? 'TBA' }}
                                                    </span>
                                                </div>
                                            </div>
                                            @if($session->capacity)
                                                <span class="shrink-0 rounded-full bg-indigo-50 px-2 py-1 text-[10px] font-bold text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-200">
                                                    {{ $session->capacity }} spots
                                                </span>
                                            @endif
                                        </div>
                                        <div class="mt-2 flex items-start gap-1.5 text-[11px] text-gray-500 dark:text-gray-400">
                                            <i class="bx bx-map mt-0.5 shrink-0"></i>
                                            <span class="min-w-0 break-words">{{ $session->venue_name ?: 'Venue to be confirmed' }}</span>
                                        </div>
                                    </div>
                                @endforeach
                            </div>
                        ...
                        @if($canPurchase)
                            <form method="POST" action="{{ route('shop.cart.add') }}" class="mt-4">
                                @csrf
                                <input type="hidden" name="type" value="plan">
                                <input type="hidden" name="id" value="{{ $p->id }}">
                                <button class="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-indigo-600 px-4 py-2.5 text-xs font-semibold text-white shadow transition hover:bg-indigo-700">
                                    <i class="bx bx-cart-add"></i> Add Plan
                                </button>
                            </form>
                        @endif
                    </div>
