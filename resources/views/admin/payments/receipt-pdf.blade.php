...
<body>
    <div class="page">
        ...
        <div class="section">
            ...
            <table class="info-table">
                <tr>
                    <td class="label">Internal Payment ID</td>
                    <td>{{ $payment->id }}</td>
                </tr>
                <tr>
        ...
        <div class="section">
            ...
            <table class="summary">
                ...
                <tr>
                    <td>Tax</td>
                    <td class="text-right">
                        {{ strtoupper($payment->currency ?? 'MYR') }} {{ number_format((float) data_get($payload, 'total_details.amount_tax', 0) / 100, 2) }}
                    </td>
                </tr>
                <tr class="total-row">
                    <td>Total Paid</td>
        ...
        <div class="section">
            <div class="section-title">Gateway Snapshot</div>
            <table class="info-table">
                @if(($payment->provider ?? null) === 'hitpay')
                    <tr>
                        <td class="label">HitPay Payment ID</td>
                        <td>{{ $payload['payment_id'] ?? '-' }}</td>
                    </tr>
                    <tr>
                        <td class="label">HitPay Request ID</td>
                        <td>{{ $payload['payment_request_id'] ?? '-' }}</td>
                    </tr>
                    <tr>
                        <td class="label">Reference Number</td>
                        <td>{{ $payload['reference_number'] ?? '-' }}</td>
                    </tr>
                    <tr>
                        <td class="label">Gateway Status</td>
                        <td>{{ $payload['status'] ?? '-' }}</td>
                    </tr>
                @elseif(($payment->provider ?? null) === 'stripe')
                    <tr>
                        <td class="label">Stripe Checkout Session</td>
                        <td>{{ $payload['id'] ?? '-' }}</td>
                    </tr>
                    <tr>
                        <td class="label">Payment Intent</td>
                        <td>{{ $payload['payment_intent'] ?? '-' }}</td>
                    </tr>
                    <tr>
                        <td class="label">Payment Status</td>
                        <td>{{ $payload['payment_status'] ?? '-' }}</td>
                    </tr>
                    <tr>
                ...
                @else
                    <tr>
                        <td class="label">Provider Payload</td>
                        <td>{{ !empty($payload) ? 'Available' : 'Not Available' }}</td>
                    </tr>
                @endif
            </table>
        </div>
