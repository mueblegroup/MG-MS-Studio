<x-app-layout>
    @php
        ...
        $teacherSeats = $seatUsage['teacher'] ?? ['used' => 0, 'limit' => null, 'unlimited' => true, 'full' => false, 'percentage' => 0];
    @endphp

    <div class="min-h-screen bg-gray-50/60 dark:bg-gray-900 p-6 sm:p-8">
        <div class="flex flex-col md:flex-row gap-4 mb-8">
            <div>
                <h1 class="text-2xl font-bold tracking-tight text-gray-900 dark:text-white">Create Teacher</h1>
                <p class="text-sm text-gray-500 dark:text-gray-400">Create a new teacher user.</p>
            </div>
        </div>
        ...
        @if($errors->any())
            <div class="mb-6 rounded-2xl border border-red-200 bg-red-50 p-4 text-red-800 dark:border-red-900/50 dark:bg-red-950/30 dark:text-red-200">
                <div class="font-bold">Teacher could not be created</div>
        ...
        @unless($teacherSeats['unlimited'])
            <div class="mb-6 rounded-2xl border {{ $teacherSeats['full'] ? 'border-red-300 bg-red-50 dark:border-red-900/60 dark:bg-red-950/30' : 'border-amber-300 bg-amber-50 dark:border-amber-900/60 dark:bg-amber-950/30' }} p-5 shadow-sm">
                <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <div class="text-sm font-extrabold text-gray-900 dark:text-white">
                            {{ $teacherSeats['full'] ? 'Teacher seat limit reached' : 'Teacher seat usage' }}
                        </div>
                        <div class="mt-1 text-sm font-semibold text-gray-700 dark:text-gray-300">
                            {{ $teacherSeats['used'] }}/{{ $teacherSeats['limit'] }} teacher seats used.
                            {{ $teacherSeats['full'] ? 'Upgrade your plan before adding another teacher.' : 'Upgrade now to unlock more teacher seats.' }}
                        </div>
                    </div>
        ...
        @if($teacherSeats['full'])
            <div class="rounded-2xl border border-gray-200 bg-white p-6 text-sm font-semibold text-gray-600 shadow-sm dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300">
                The create form is disabled because all teacher seats are currently occupied.
            </div>
        @else
